...
class TestReservationSystem(unittest.TestCase):
    ...
    @patch('reservation_system.management_hotel.HotelManagement')
    @patch('reservation_system.management_customer.CustomerManagement')
    @patch('reservation_system.management_reservation.ReservationManagement')
    ...
    def test_exit(self, mock_stdout, mock_input):
        with self.assertRaises(SystemExit):
            run_reservation_system(MagicMock(), MagicMock(), MagicMock())
